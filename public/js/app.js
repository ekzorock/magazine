...
www('.submenu3', '.sublist3'); // Открываем корзину по клику

document.querySelector('.shop_cart-cart').addEventListener("click", function () {
  var popup = document.getElementById("myPopup");
  popup.classList.remove("ofshow");
  popup.classList.toggle("show");
  var shop_cart = document.querySelector('.shop_cart-cart');
  shop_cart.classList.toggle("displayNone"); //   Удаление из Локалки после заказа
...
document.onclick = function (event) {
  if (event.target.classList.contains('basketBut')) {
    //если(у таргета класс.содержит basketbut)
    var itemCart = event.target.classList['1']; // отслеживаем айдишник по которому щелкнули

    var _www = document.querySelectorAll('.itemList'); // получаем список товаров


    if (localStorage.getItem('muse')) {
      // если в локалсторедж что то есть
      var objec = JSON.parse(localStorage.getItem("muse")); //подгружаем из локал сторедж

      for (var i = 0; i < _www.length; i++) {
        ...
        if (_www[i].id == itemCart) {
          ...
          objec[_www[i].dataset.article] = {
            ...
            "price": +_www[i].dataset.price,
            "count": 1
          };
          var serialObj = JSON.stringify(objec); // парсим в строку

          localStorage.setItem("muse", serialObj); //записываем в локал
        }
      }
    } else {
      // если в локал сторедж ничего нет
      var _objec = {};
      ...
      for (var _i = 0; _i < _www.length; _i++) {
        if (_www[_i].id == itemCart) {
          _objec[_www[_i].dataset.article] = {
            ...
            "price": +_www[_i].dataset.price,
            "count": 1
          };

          var _serialObj = JSON.stringify(_objec);

          localStorage.setItem("muse", _serialObj);
        }
      }
    }
  }
...
var cartImg = document.querySelector('.bascetCart');
document.querySelector('.shop_cart-cart').addEventListener("click", function rend() {
  ...
  document.onclick = function (event) {
    ...
    if (event.target.classList.contains('butPlus')) {
      var _localMuse = JSON.parse(localStorage.getItem("muse")); // получаем объект с локалсторедж


      var itemCart = event.target.dataset; // отслеживаем айдишник по которому щелкнули

      _localMuse[itemCart.plus].count++;
      var serialObj = JSON.stringify(_localMuse);
      localStorage.setItem("muse", serialObj);
      cartImg.innerHTML = '';
      rend(); // обновляем корзину из локалсторедж

      fullPriceFunc(); // рендерим сумму ВСЕГО
    } else if (event.target.classList.contains('butMinus')) {
      // минусуем
      var _localMuse2 = JSON.parse(localStorage.getItem("muse")); // получаем объект с локалсторедж


      var _itemCart = event.target.dataset; // отслеживаем айдишник по которому щелкнули

      _localMuse2[_itemCart.minus].count--;

      var _serialObj2 = JSON.stringify(_localMuse2);

      localStorage.setItem("muse", _serialObj2);
      cartImg.innerHTML = '';
      rend(); // обновляем корзину из локалсторедж
...
function fullPriceFunc() {
  ...
  var localMuse = JSON.parse(localStorage.getItem("muse")); // получаем объект с локалсторедж
...
function objectDelete() {
  ...
  for (var key in localMuse) {
    if (localMuse[key].count < 1) {
      ...
      var serialObj = JSON.stringify(localMuse);
      localStorage.setItem("muse", serialObj);
      cartImg.innerHTML = '';

      for (var _key in localMuse) {
        cartImg.innerHTML += "<div class=\"bascetCartRend\">\n                        <div class=\"bascetCart-article\">\u0410\u0440\u0442\u0438\u043A\u0443\u043B: ".concat(localMuse[_key].article, "</div>\n                        <div class=\"bascetCart-img\"><img src=\"").concat(localMuse[_key].img, "\" alt=\"img\" srcset=\"\"></div>\n                        <div class=\"bascetCart-name\">").concat(localMuse[_key].name, "</div>\n                        <div><button class=\"butMinus\" data-minus=\"").concat(localMuse[_key].article, "\">-</button></div>\n                        <div class=\"bascetCart-count\">\u041A\u043E\u043B-\u0432\u043E: ").concat(localMuse[_key].count, "</div>\n                        <div><button class=\"butPlus\" data-plus=\"").concat(localMuse[_key].article, "\">+</button></div>\n                        <div class=\"bascetCart-price\">\u0426\u0435\u043D\u0430: ").concat(localMuse[_key].price * localMuse[_key].count, "</div> \n                    </div>");
      }
    }
  }
...
document.querySelector('.closeBascet').addEventListener('click', function () {
  console.log('close');
  cartImg = '';
  var popup = document.getElementById("myPopup");
...
/***/ "./resources/sass/mobileNavi.scss":
/*!****************************************!*\
  !*** ./resources/sass/mobileNavi.scss ***!
  \****************************************/
/*! no static exports found */
...
/***/ 0:
/*!*********************************************************************************************************************************************************!*\
  !*** multi ./resources/js/app.js ./resources/sass/app.scss ./resources/sass/about.scss ./resources/sass/adaptive.scss ./resources/sass/mobileNavi.scss ***!
  \*********************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
__webpack_require__(/*! C:\OSPanel\domains\magazine\resources\sass\about.scss */"./resources/sass/about.scss");
__webpack_require__(/*! C:\OSPanel\domains\magazine\resources\sass\adaptive.scss */"./resources/sass/adaptive.scss");
module.exports = __webpack_require__(/*! C:\OSPanel\domains\magazine\resources\sass\mobileNavi.scss */"./resources/sass/mobileNavi.scss");
